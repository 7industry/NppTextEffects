   ...
   class KanaConverter
    {
        ...
        {
            #region
            public static readonly uint FELANG_REQ_CONV = 0x00010000;   // 変換（ひらがなを漢字に変換）
            public static readonly uint FELANG_REQ_RECONV = 0x00020000; // 再変換（カタカナや半カナも漢字に変換）
            public static readonly uint FELANG_REQ_REV = 0x00030000;   // 逆変換（読み仮名に変換）
            ...
            public string GetKatakana(string str)
            {
                ...
                return this.GetJMorphResult(str, FELANG_REQ_REV, FELANG_CMODE_KATAKANAOUT);
            }
        ...
        {
            // 用意
            string kanzi = "冬過ぎて春来るらし朝日さす春日の山に霞たなびく";
            ...
            try
            {
                ...
                ife.Open();
                //ife.GetPhonetic(kanzi, 1, -1, out string hiragana);
                ife.GetPhonetic(kanzi, 1, -1, out string hiragana);
                System.Console.WriteLine($"{kanzi} => {hiragana}"); // 漢字 => かんじ
                ife.GetConversion(hiragana, 1, -1, out kanzi);
                System.Console.WriteLine($"{hiragana} => {kanzi}"); // かんじ => 漢字
            }
            finally
            {
                ife?.Close();
